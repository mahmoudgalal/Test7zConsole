	...
	//    	boolean codecEncoderIsAssigned,String codecName)
     ...
     for (UInt32 j = 0; j < numMethods; j++)
     {
      ...
      // LOGI(" %s %s %s",(codecs->GetCodecEncoderIsAssigned(j) ? "C" : " ")
       ...
       env->CallVoidMethod(obj,addSupportedCodec_ID,clibIndex,codecId,codecEncoderIsAssigned
    		   ,codecName);
       if(codecName)
...
int InitializeUpdateCallbackIds(JNIEnv * env)
{
	...
	addErrorMessage = env->GetMethodID(updateCallbackClass, "addErrorMessage", "(Ljava/lang/String;)V");
	if(!addErrorMessage)
  ...
  (JNIEnv * env, jclass cls)
{
	...
	openResult = env->GetMethodID(extractCallbackClass, "openResult", "(Ljava/lang/String;JZ)V");
	 ...
	 setCurrentFilePath = env->GetMethodID(extractCallbackClass, "setCurrentFilePath", "(Ljava/lang/String;J)J");
	 ...
	 exAddErrorMessage = env->GetMethodID(extractCallbackClass, "addErrorMessage", "(Ljava/lang/String;)V");
...
JNIEXPORT jint JNICALL Java_com_aroma_zeearchiver_Archive_createArchive
  (JNIEnv *env, jobject obj, jstring archivename, jobjectArray filespaths,jint length,
		  ...
		  ,jboolean encryptHeadersAllowed,jstring pass
		  ,jboolean multiThread,jobject callback)
{
	...
	args[0]= "7z";args[1]= "a";args[2]= "-y";args[3]=arcName;
	for(int i=0;i<length;i++)
		args[i+4]= files[i];
	CInfo Info ;
